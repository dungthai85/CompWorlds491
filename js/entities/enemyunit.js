function EnemyUnit(game, ENTITY_NAME, POSITION, LEVEL) {
	...
	if (ENEMY.TYPE === "Boss") {
		...
		this.dead_animation = new MyAnimation(ENEMY.sprite_sheet, 0, 1200, 400, 250, 0.2, 10, false, false);
		this.FORM_SCALE = 1.0;
		...
		this.walk_animation = new MyAnimation(ENEMY.sprite_sheet, 0, 0, 400, 250, 0.05, 10, true, false);
		this.attack_animation = new MyAnimation(ENEMY.sprite_sheet, 0, 400, 400, 250, 0.05, 10, true, false);
		this.hurt_animation = new MyAnimation(ENEMY.sprite_sheet, 0, 800, 400, 250, 0.1, 10, true, false);
		this.dead_animation = new MyAnimation(ENEMY.sprite_sheet, 0, 1200, 400, 250, 0.2, 10, false, false);
		this.FORM_SCALE = 0.7;
		// this.effect = new MyAnimation(AM.getAsset("./img/Others/hit_effect.png"), 0, 0, 1024, 1024, 0.03, 16, true, false);
	} else {
	...
	this.boundingbox = new BoundingBox(this.x + 20, this.y + 20, 3, this.attack_animation.frameHeight * 0.2);
...
EnemyUnit.prototype.update = function() {
	...
	// Update animation
	if (this.name === "TrollGuard" && this.x < 1000) {
		this.moving = false;
		this.taunt = true;
		this.attacking = false;
	}
	...
	if (this.name === "TrollWarlord") {
		this.boundingbox = new BoundingBox(this.x + 120, this.y + 150, 3, this.attack_animation.frameHeight * 0.1);
		this.hp_bar = new EnemyHP(this.x + 150, this.y + 230, this.hp_scale * 3 - (this.hp - this.hp_current) * ((this.hp_scale * 3) / this.hp), 15);
	} else if (this.name === "TrollGuard") {
		this.boundingbox = new BoundingBox(this.x + 50, this.y + 80, 3, this.attack_animation.frameHeight * 0.15);
		// this.hp_bar = new EnemyHP(this.x + 150, this.y + 230, this.hp_scale * 3 - (this.hp - this.hp_current) * ((this.hp_scale * 3) / this.hp), 15);
	} else {
		this.boundingbox = new BoundingBox(this.x + 20, this.y + 20, 3, this.attack_animation.frameHeight * 0.2);
...
EnemyUnit.prototype.draw = function() {
	...
	if (this.moving && this.hp_current > 0) {
		...
		if (this.name === "TrollWarlord") {
			this.effect.drawFrame(this.game.clockTick, this.ctx, this.x - 30, this.y, 0.3);
		} else if (this.name === "TrollGuard") {
			this.effect.drawFrame(this.game.clockTick, this.ctx, this.x - 30, this.y, 0.3);
		} else {
			this.effect.drawFrame(this.game.clockTick, this.ctx, this.x - 30, this.y, 0.1);
		}
	...
	if (this.taunt === true) this.hurt_animation.drawFrame(this.game.clockTick, this.ctx, this.x, this.y, this.FORM_SCALE);
	// Draw hp bar background
	if (this.name !== "TrollGuard") {
...
function Enemy_Generator(ENTITY_NAME) {
	...
	} else if (ENTITY_NAME === "TrollGuard") {
		enemy = {
			sprite_sheet: AM.getAsset("./img/enemy_team/boss/troll_warlord.png"),
			HP: 2500 * MULTIPLY,
			DAMAGE: 1,
